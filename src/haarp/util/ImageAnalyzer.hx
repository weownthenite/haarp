...
class ImageAnalyzer {
    ...
    public static function calculateBrightness( rgb : Array<Int> ) : Int {
        return ((rgb[0] & 0xFF) << 16) | ((rgb[1] & 0xFF) << 8) | ((rgb[2] & 0xFF) << 0);
    }

    #if nodejs

    public static function getAverageBrightness( path : String, callback : Error->Int->Void ) {
        getAverageColors( path, function(e,rgb) {
            if( e != null ) {
                callback( e, null );
            } else {
                callback( null, calculateBrightness( rgb) );
            }
        });
    }
